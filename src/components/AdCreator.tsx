...
export const AdCreator: React.FC<Props> = ({ onSave, initialData }) => {
  ...
  // Standardized accessibility IDs
  const nameId = useId();
  const bodyId = useId();
  ...
  const getCounterColor = (current: number, limit: number) => {
    if (current > limit) return 'text-red-500 font-bold';
    if (current >= limit * 0.8) return 'text-amber-500 font-medium';
      ...
      <div className="space-y-6 bg-zinc-950/60 p-8 rounded-3xl border border-white/5">
        <div className="flex items-center justify-between mb-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-[10px] font-bold text-indigo-400 hover:bg-indigo-500/20 disabled:opacity-50 disabled:cursor-not-allowed transition-all cursor-pointer"
          >
            {isGenerating ? (
              <>
        ...
        <div className="space-y-4">
          <div>
            <label htmlFor={nameId} className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-1.5 cursor-pointer">
              Ad Name
            </label>
            <input
              id={nameId}
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              ...
              <span className={`text-[10px] ${getCounterColor(bodyLength, 125)}`}>
                {bodyLength}/125
              </span>
            </div>
            <textarea
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              <label htmlFor={headlineId} className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block cursor-pointer">
                Headline
              </label>
              <span className={`text-[10px] ${getCounterColor(headlineLength, 40)}`}>
                {headlineLength}/40
              </span>
            </div>
            <input
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor={ctaId} className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-1.5 cursor-pointer">
                Call to Action
              </label>
              <select
                id={ctaId}
