...
import { Wand2, RefreshCw, Image as ImageIcon, Layout, Type } from 'lucide-react';
...
export const AdCreator: React.FC<Props> = ({ onSave, initialData }) => {
  const [ad, setAd] = useState<Partial<MetaAd>>(initialData || {
    name: '', headline: '', body: '', cta: 'Learn More', imageUrl: ''
  });
      ...
      <div className="space-y-6 bg-zinc-950/60 p-8 rounded-3xl border border-white/5">
        <div className="flex items-center justify-between mb-2">
          ...
          <button
            ...
            aria-label={isGenerating ? "Generating ad copy..." : "Magic generate ad copy"}
        ...
        <div className="space-y-4">
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              ...
              <span className={`text-[10px] font-mono ${(ad.body?.length || 0) > 125 ? 'text-red-400' : (ad.body?.length || 0) >= 100 ? 'text-orange-400' : 'text-gray-500'}`}>{ad.body?.length || 0}/125</span>
            </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              ...
              <span className={`text-[10px] font-mono ${(ad.headline?.length || 0) > 40 ? 'text-red-400' : (ad.headline?.length || 0) >= 32 ? 'text-orange-400' : 'text-gray-500'}`}>{ad.headline?.length || 0}/40</span>
            </div>
