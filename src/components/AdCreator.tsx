      ...
      <div className="space-y-6 bg-zinc-950/60 p-8 rounded-3xl border border-white/5">
        <div className="flex items-center justify-between mb-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-[10px] font-bold text-indigo-400 hover:bg-indigo-500/20 transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
          >
        ...
        <div className="space-y-4">
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              ...
              <span className={`text-[10px] font-mono ${(ad.body?.length || 0) > 125 ? 'text-red-400 font-bold' : (ad.body?.length || 0) > 100 ? 'text-orange-400' : 'text-gray-500'}`}>
                {ad.body?.length || 0}/125
              </span>
            </div>
            <textarea
              id={bodyId} name="body" rows={4} value={ad.body || ''} onChange={handleChange}
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              ...
              <span className={`text-[10px] font-mono ${(ad.headline?.length || 0) > 40 ? 'text-red-400 font-bold' : (ad.headline?.length || 0) > 32 ? 'text-orange-400' : 'text-gray-500'}`}>
                {ad.headline?.length || 0}/40
              </span>
            </div>
            <input
              id={headlineId} name="headline" value={ad.headline || ''} onChange={handleChange}
              className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-sm text-white focus:border-emerald-500 focus:outline-none"
