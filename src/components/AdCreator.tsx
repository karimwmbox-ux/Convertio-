      ...
      <div className="space-y-6 bg-zinc-950/60 p-8 rounded-3xl border border-white/5">
        <div className="flex items-center justify-between mb-2">
          ...
          <button
            ...
            aria-label="Magic Generate AI copy"
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-[10px] font-bold text-indigo-400 hover:bg-indigo-500/20 disabled:opacity-50 transition-all cursor-pointer"
          >
        ...
        <div className="space-y-4">
          <div>
            <label htmlFor="ad-name" className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-1.5 cursor-pointer">Ad Name</label>
            <input
              id="ad-name"
              name="name"
              value={ad.name}
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              <label htmlFor="ad-body" className="text-[10px] font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1.5 cursor-pointer">
                <Type className="w-3 h-3" /> Primary Text (Body)
              </label>
              <span className={`text-[10px] font-mono ${(ad.body?.length || 0) > 125 ? 'text-red-400' : 'text-gray-500'}`}>{ad.body?.length || 0}/125</span>
            </div>
            <textarea
              id="ad-body"
              name="body"
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              <label htmlFor="ad-headline" className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block cursor-pointer">Headline</label>
              <span className={`text-[10px] font-mono ${(ad.headline?.length || 0) > 40 ? 'text-red-400' : 'text-gray-500'}`}>{ad.headline?.length || 0}/40</span>
            </div>
            <input
              id="ad-headline"
              name="headline"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="ad-cta" className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-1.5 cursor-pointer">Call to Action</label>
              <select
                id="ad-cta"
                name="cta"
                value={ad.cta}
            ...
            <div>
              <label htmlFor="ad-image-url" className="text-[10px] font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1.5 mb-1.5 cursor-pointer">
