...
export const AdCreator: React.FC<Props> = ({ onSave, initialData }) => {
  ...
  const getCharacterCountColor = (current: number, limit: number) => {
    ...
    if (current > limit * 0.8) return 'text-orange-400';
    return 'text-gray-500';
  };

  const handleGenerateCopy = () => {
    setIsGenerating(true);
    // Simulating AI generation with an artificial delay for UX "delight"
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <button
            ...
            className={cn(
              ...
              isGenerating
                ? "bg-indigo-500/20 border border-indigo-500/30 text-[10px] font-bold text-indigo-300 cursor-not-allowed"
                : "bg-indigo-500/10 border border-indigo-500/20 text-[10px] font-bold text-indigo-400 hover:bg-indigo-500/20"
            )}
          >
            {isGenerating ? (
        ...
        <div className="space-y-4">
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              ...
              <span className={cn("text-[9px] font-bold uppercase tracking-tight", getCharacterCountColor((ad.body || '').length, 125))}>
                {(ad.body || '').length} / 125
              </span>
            </div>
            <textarea
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              ...
              <span className={cn("text-[9px] font-bold uppercase tracking-tight", getCharacterCountColor((ad.headline || '').length, 40))}>
