...
export const AdCreator: React.FC<Props> = ({ onSave, initialData }) => {
  ...
  const handleGenerateCopy = () => {
    ...
    // Simulating AI generation with UX delight delay
    setTimeout(() => {
      setAd(prev => ({
        ...prev,
  ...
  const handleSave = () => {
    ...
    // Simulating save operation with UX delight delay
    setTimeout(() => {
      onSave(ad);
      setIsSaving(false);
    }, 800);
    ...
    <div className="grid lg:grid-cols-2 gap-8 items-start">
      ...
      <div className="space-y-6 bg-zinc-950/60 p-8 rounded-3xl border border-white/5">
        ...
        <button
          onClick={handleSave}
          disabled={isSaving}
          className="w-full py-4 rounded-2xl bg-emerald-500 text-black font-black text-sm hover:bg-emerald-400 transition-all active:scale-95 mt-4 flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
        >
