    ...
    <div className="grid lg:grid-cols-2 gap-8 items-start">
      ...
      <div className="space-y-6 bg-zinc-950/60 p-8 rounded-3xl border border-white/5">
        ...
        <div className="space-y-4">
          <div>
            <label htmlFor={id+'n'} className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-1.5 cursor-pointer">Ad Name</label>
            <input id={id+'n'} name="name" value={ad.name} onChange={handleChange}
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              <label htmlFor={id+'b'} className="text-[10px] font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1.5 cursor-pointer"><Type className="w-3 h-3" /> Primary Text (Body)</label>
              <span className={`text-[9px] font-bold ${(ad.body?.length||0)>125?"text-red-500":(ad.body?.length||0)>=100?"text-orange-500":"text-gray-500"}`}>{ad.body?.length||0}/125</span>
            </div>
            <textarea id={id+'b'} name="body"
              rows={4}
              value={ad.body}
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              <label htmlFor={id+'h'} className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block cursor-pointer">Headline</label>
              <span className={`text-[9px] font-bold ${(ad.headline?.length||0)>40?"text-red-500":(ad.headline?.length||0)>=32?"text-orange-500":"text-gray-500"}`}>{ad.headline?.length||0}/40</span>
            </div>
            <input id={id+'h'} name="headline" value={ad.headline} onChange={handleChange}
              className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-sm text-white focus:border-emerald-500 focus:outline-none"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor={id+'c'} className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-1.5 cursor-pointer">Call to Action</label>
              <select id={id+'c'} name="cta" value={ad.cta} onChange={handleChange}
            ...
            <div>
              <label htmlFor={id+'i'} className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-1.5 flex items-center gap-1.5 cursor-pointer">
                <ImageIcon className="w-3 h-3" /> Image URL
              </label>
              <input id={id+'i'} name="imageUrl" value={ad.imageUrl} onChange={handleChange}
                className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-sm text-white focus:border-emerald-500 focus:outline-none"
