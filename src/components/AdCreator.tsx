    ...
    <div className="grid lg:grid-cols-2 gap-8 items-start">
      ...
      <div className="space-y-6 bg-zinc-950/60 p-8 rounded-3xl border border-white/5">
        ...
        <div className="space-y-4">
          <div>
            <label htmlFor={nameId} className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-1.5 cursor-pointer hover:text-gray-300 transition-colors">Ad Name</label>
            <input
              id={nameId}
              name="name"
              value={ad.name}
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              <label htmlFor={bodyId} className="text-[10px] font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1.5 cursor-pointer hover:text-gray-300 transition-colors">
                <Type className="w-3 h-3" /> Primary Text (Body)
              </label>
              <span className={`text-[10px] font-medium font-mono ${
                (ad.body || '').length > 125 ? 'text-red-400' : (ad.body || '').length >= 100 ? 'text-orange-400' : 'text-gray-500'
              }`}>
                {(ad.body || '').length}/125
              </span>
            </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              <label htmlFor={headlineId} className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block cursor-pointer hover:text-gray-300 transition-colors">Headline</label>
              <span className={`text-[10px] font-medium font-mono ${
                (ad.headline || '').length > 40 ? 'text-red-400' : (ad.headline || '').length >= 32 ? 'text-orange-400' : 'text-gray-500'
              }`}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor={ctaId} className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-1.5 cursor-pointer hover:text-gray-300 transition-colors">Call to Action</label>
              <select
                id={ctaId}
                name="cta"
                value={ad.cta}
            ...
            <div>
              <label htmlFor={imageUrlId} className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-1.5 flex items-center gap-1.5 cursor-pointer hover:text-gray-300 transition-colors">
